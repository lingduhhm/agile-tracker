...
<template>
  <div>
    <el-card class="box-card">
      <div id="estimationChartLight" style=""></div>
    </el-card>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    mounted () {
      var that = this;
      this.myChart = echarts.init(document.getElementById('estimationChartLight'));
    ...
    methods: {
      fetchData: function (isRefresh, sprintinfo) {
        ...
        var loading = Loading.service({fullscreen: true,
          ...
          text: 'Loading...'});
        var that = this;
        var options = 'refresh=' + (isRefresh || '') + '&sprintid=' + (this.sprintinfo._id || '') + '&start=' + (this.sprintinfo.start || '') + '&end=' + (this.sprintinfo.end || '') + '&module=' + this.module;
